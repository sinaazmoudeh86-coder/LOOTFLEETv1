# LAUNCH — deploy-v215 · build 452

Stamps verified aligned: `version.json` **452** · `LF_BUILD` **452** · SW cache `lootfleet-v452`.
...
Because `sw.js` cache is bumped to `lootfleet-v452`, returning players get fresh assets rather
than a cached 451 mix.
...
1. **Load the game.** Confirm the build gate lets you in — `version.json` and `LF_BUILD` both
   read 452, which is the check that locked everyone out at v214.
...
3. **Space Casino.** The three holds appear under the bet chips showing `1%`, `2%`, `3%` and
   `LV 100/300/500` — **not** `⋯ LEDGER OFFLINE`. If they read offline, step 2 didn't take.
...
**New — Casino house citadels.** Three holds paying 1% / 2% / 3% of *server-wide* daily losses,
all five currencies including LootCoins. Gated Lv 100/300/500, one hold per pilot, 24h shield
on capture. Payout 00:10 UTC, delivered as labelled claimable mail. Discord gets owners and
totals in the 3-hourly report plus immediate big-bet callouts (tiered per currency, rate
limited to one per pilot per 90s unless the next round doubles it).
