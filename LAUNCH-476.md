# LAUNCH — deploy-v215 · build 476

Stamps verified aligned: `version.json` **476** · `LF_BUILD` **476** · SW cache `lootfleet-v476`.
...
**OWNED VOID GATE (476).** The void level gate now holds for tiles you OWN — ascension resets
level but kept warp access to your own deep spires, an easy-XP exploit. Income and claims are
untouched; re-earn the level to fly there. Casino holds inherit the gate.

**CHOIR ZONE VISIBILITY (475).** Choir zones always ran past 500 (533, 592, 614… forever); the
zone list just stops at your front line +3. The briefing now shows the next 12 Choir zones with
garrison tiers.

**THE CRASH WAVE — ROOT CAUSE FOUND AND FIXED (474).** Item generation picks DISTINCT stats
from a pool of SIX core stats, but Celestial declares 7 and Paragon 7–8 — the pick loop could
never finish, freezing the tab the instant a top-tier item dropped. The loot rebalance is what
made those tiers obtainable, which is why the wave started when it did, hit players anywhere
loot drops, and even hung some players AT LOGIN (the offline sim rolls items during boot).
Matches every banner: tiny heap, healthy arrays, sudden freeze, no stall warning (an infinite
loop never yields another frame). Stat count now caps at the pool; the tier's power stays in
its multiplier. Deploy 474 IMMEDIATELY.

**MOVE SPEED CAP + HANG FIXES (473, kept).** Move speed capped at +1000% (was reaching 3541%).
Audit found two speed-adjacent hangs: moveToward had no overshoot clamp (at extreme speed the
ship oscillated across its target every frame, thrashing camera + collision), and the two
facing-normalisation while-loops hang the tab FOREVER if the angle ever goes non-finite —
exactly matching the crash banner profile (healthy sample, sudden death, marker stuck at
'alive'). Both are now loop-free/clamped. Storm chain loop and render caches audited: bounded.

**LOGIN BUILD STAMP (472, kept).** The auth screen now shows `BUILD 472` above Create account /
Log in, wired to LF_BUILD. After deploying, open lootfleet.com logged out: if it does not say
472, the deploy is not propagating (hosting cache / SW) and that is the crash-report gap.

**LIVE STALL DETECTOR (471) — the crash net that doesn't need a relog.** Every prior fix
depended on the NEXT login showing evidence, and players relog into fresh tabs where nothing
survived. Now a >1.5s frame stall (twice in 60s, or any single 4s stall) triggers recovery IN
SESSION: arrays hard-trimmed, amber banner shown immediately with build number + live array
sizes + heap. One screenshot names the culprit while the tab is still alive. Also: a 1.4GB heap
ceiling triggers the same recovery, and real JS exceptions store their message for the next
boot's banner. IMPORTANT: the banner prints the running build — if a crashing player has no
banner at all, or it shows < 471, the deploy is NOT reaching them (stale hosting or cache), and
that is the problem to chase.

**RECOVERY BANNER ORDERING FIX (469, kept).** load() stamped the boot marker before init read the
