...
(function () {
  ...
  // PINNED SESSION — read io-auth ONCE and hold it for this tab's lifetime.
  // localStorage is shared by every tab of the same browser, so re-reading it
  // at save-time let a sign-in in tab B silently redirect tab A's autosaves
  // into the NEW account's slot (cross-account bleed). Each tab now keeps the
  // account it booted with; auth.js calls rebind() on its own sign-in/out.
  // Running DIFFERENT accounts in different tabs/browsers is fully supported.
  let _pin = null;
  function rebind() { try { _pin = JSON.parse(localStorage.getItem(SESS)); } catch (e) { _pin = null; } return _pin; }
  function session() { return _pin || rebind(); }
  function uid() {
    const s = session();
  ...
  window.ACCOUNT = { key, uid, current, session, rebind, load, save: saveLocal, push, pull, flushNow, refreshBar, cloudOn, setName };
})();
