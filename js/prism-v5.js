...
(function () {
  ...
  const MINERS = {
    scout:     { name: 'Scout Miner',     cost: 600,    rate: 1.6, hp: 320,  grow: 1.4, sprite: 'interceptor',  col: '#9aa7b8', blurb: 'Cheap, low yield.' },
    hauler:    { name: 'Hauler Rig',      cost: 5200,   rate: 3.6, hp: 900,  grow: 1.4, sprite: 'cruiser',      col: '#5fd1ff', blurb: 'Balanced workhorse.' },
    heavy:     { name: 'Heavy Excavator', cost: 32000,  rate: 7.5, hp: 2600, grow: 1.4, sprite: 'heavycruiser', col: '#d0a060', blurb: 'Armoured, strong output.' },
    harvester: { name: 'Prism Harvester', cost: 180000, rate: 15,  hp: 7200, grow: 3.0, sprite: 'carrier',      col: '#ffd450', blurb: 'Endgame rig. Huge yield.' },
  };
  ...
  function ratePerSec(tier) { let r = 0; P().miners.forEach((m) => r += MINERS[m.type].rate); return r * tierMult(tier) * coreMult() * refMult(); }
  ...
  function tick(dt, rt) {
    ...
    // production
    if (RUN.ore > 0 && rate > 0) {
      const prod = Math.min(RUN.ore, rate * tierMult(RUN.tier) * coreMult() * refMult() * dt);
