...
(function () {
  ...
  function openSheet() {
    ...
    _m.innerHTML = '<div class="vipm-back"></div><div class="vipm-card">' +
      ...
      '<div class="vipm-now">Active now: <b>+' + (GOLD[Math.min(lv, 15)] || 0) + '% gold · +' + (XP[Math.min(lv, 15)] || 0) + '% XP · +' + (AFK[Math.min(lv, 15)] || 0) + '% AFK' + (lv > 7 ? ' · +' + (lv - 7) * 2 + '% ◈ Prism' : '') + (lv > 0 ? ' · +' + lv * 5 + ' citadel cap' : '') + '</b></div>' +
