...
(function () {
  ...
  function setup() {
    ...
    canvases.forEach(function (cv) {
      ...
      const rec = { cv, ctx, api, size, name: cv.getAttribute('data-sim'), scene: null, visible: false };
      sims.push(rec);
      if (io) io.observe(cv);
    });
    ...
    let pending = false;
    function queue() { if (pending) return; pending = true; requestAnimationFrame(function (n) { pending = false; loop(n); }); }
    queue();
    setInterval(function () { if (performance.now() - last > 130) loop(performance.now()); }, 130);
