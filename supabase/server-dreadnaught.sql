...
create table if not exists public.sdread_scores (
  ...
  best_day   bigint not null default 0,          -- best single run that day
  total      bigint not null default 0,          -- season cumulative damage
...
create or replace function public.sdread_upsert(
  p_name text, p_season int, p_day int, p_best bigint, p_total bigint, p_stage int
...
grant execute on function public.sdread_upsert(text, int, int, bigint, bigint, int) to authenticated;
